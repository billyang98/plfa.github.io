...
```
module plfa.part2.Lambda_solutions where
```
...
#### Exercise `mul` (recommended) ****HOMEWORK****
...
#### Exercise `mulᶜ` (practice) ****HOMEWORK****
...
The definition of —↠′ for trans is different and does not rely on
a step as in —↠ for transitivity. Thus it can be associated differently
...
—↠-trans :  ∀ {L M N}
    ...
    → M —↠ N
      ---------
    → L —↠ N
—↠-trans {L} {M} {N} (M ∎) m—↠n = m—↠n
—↠-trans {L} {M} {N} (L —→⟨ L→L ⟩ L—↠N ) M—↠N = L —→⟨ L→L ⟩ (—↠-trans L—↠N M—↠N)
...
—↠≲—↠′from∘to : ∀ {M} {N} (x : M —↠ N)
  → —↠≲—↠′from (—↠≲—↠′to x) ≡ x
—↠≲—↠′from∘to (M ∎) = refl
—↠≲—↠′from∘to (L —→⟨ x ⟩ x₁) rewrite —↠≲—↠′from∘to x₁ = refl
...
#### Exercise `plus-example` (practice) ****HOMEWORK****
...
#### Exercise `Context-≃` (practice) ****HOMEWORK****
...
```
-- Your code goes here
context-to : Context → List (Id × Type)
context-to ∅ = []
context-to (c , m ⦂ n ) =  (m , n) ∷ context-to c 

context-from : List (Id × Type) → Context
context-from [] = ∅
context-from ((m , n) ∷ xs) = (context-from xs , m ⦂ n )

context-fromto : (x : Context) → context-from (context-to x) ≡ x
context-fromto ∅ = refl
context-fromto (c , m ⦂ n ) rewrite context-fromto c = refl  

context-tofrom : (y : List (Id × Type)) → context-to (context-from y) ≡ y
context-tofrom [] = refl
context-tofrom ((m , n) ∷ xs) rewrite context-tofrom xs = refl
...
Context-≃ =
  record
    { to = context-to
    ; from = context-from 
    ; from∘to = context-fromto
    ; to∘from = context-tofrom
    }
```

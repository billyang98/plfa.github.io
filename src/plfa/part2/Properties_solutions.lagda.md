...
```
module plfa.part2.Properties_solutions where
```
...
#### Exercise `Progress-≃` (practice) ****HOMEWORK****

Show that `Progress M` is isomorphic to `Value M ⊎ ∃[ N ](M —→ N)`.

```
-- Your code goes here
Progress-≃-to : ∀ {M} → Progress M → Value M ⊎ ∃[ N ](M —→ N)
Progress-≃-to {M} (step {N} m→n) = inj₂ ⟨ N , m→n ⟩
Progress-≃-to {M} (done m) = inj₁ m

Progress-≃-from :  ∀ {M} → Value M ⊎ ∃[ N ](M —→ N) → Progress M
Progress-≃-from (inj₁ m) = done m
Progress-≃-from (inj₂ ⟨ n , mn ⟩) =  step mn

Progress-≃-fromto : ∀ {M} → (x : Progress M) → Progress-≃-from (Progress-≃-to x) ≡ x
Progress-≃-fromto (done m) = refl
Progress-≃-fromto (step m) = refl

Progress-≃-tofrom : ∀ {M} →  (y : Value M ⊎ ∃-syntax (_—→_ M)) →
    Progress-≃-to (Progress-≃-from y) ≡ y
Progress-≃-tofrom (inj₁ m) = refl
Progress-≃-tofrom (inj₂ ⟨ n , mn ⟩) = refl
...
Progress-≃ =
  ...
  { to = Progress-≃-to 
  ; from = Progress-≃-from
  ; from∘to = Progress-≃-fromto
  ; to∘from = Progress-≃-tofrom
  }
```
...
#### Exercise `value?` (practice) ****HOMEWORK****

Combine `progress` and `—→¬V` to write a program that decides
whether a well-typed term is a value:
```
value? : ∀ {A M} → ∅ ⊢ M ⦂ A → Dec (Value M)
value? ma with progress ma
... | done m = yes m
... | step m = no (—→¬V m) 
```
...
#### Exercise `mul-eval` (recommended) ****HOMEWORK****

Using the evaluator, confirm that two times two is four.

```
-- Your code goes here
--- mul and ⊢mul from my Lambda Homework
mul : Term
mul = μ "*" ⇒ ƛ "m" ⇒  ƛ "n" ⇒
        case ` "m"
          [zero⇒ `zero
...
_ : eval (gas 100) ⊢2*2 ≡
 ...
 —→⟨ β-suc (V-suc V-zero) ⟩
  ...
  —→⟨ ξ-suc (ξ-·₁ (β-ƛ (V-suc V-zero))) ⟩
    `suc
    (((ƛ "n" ⇒
       case `suc `zero [zero⇒ ` "n" |suc "m" ⇒ `suc (` "+" · ` "m" · ` "n") ])
      [ "+" :=
      μ "+" ⇒
      (ƛ "m" ⇒
       (ƛ "n" ⇒
...
#### Exercise: `progress-preservation` (practice) ****HOMEWORK****
...
-- Your code goes here
postulate 
  Progressp : ∀ {M A} → ∅ ⊢ M ⦂ A  → Value M ⊎ ∃[ N ](M —→ N)

postulate
  pPreservation : ∀ {M A N} → ∅ ⊢ M ⦂ A → M —→ N →  ∅ ⊢ N ⦂ A
```


#### Exercise `subject_expansion` (practice) ****HOMEWORK****
...
```
 ...
 `zero
 which is `ℕ
 but `zero · `zero is not well-typed
 so  (ƛ "m" ⇒ `zero) · (`zero · `zero) is not well-typed


 case `zero [zero⇒ `zero |suc "m" ⇒ (`zero · `zero)]
 reduces to
 `zero
  which is `ℕ
  but similarly `zero · `zero is not well-typed
  so the original case expression is not well-typed
...
#### Exercise `stuck` (practice) ****HOMEWORK****

Give an example of an ill-typed term that does get stuck.

```
-- Your code goes here
notvalueex :  ¬ Value (`zero · `zero)
notvalueex ()

normalex : Normal (`zero · `zero)
normalex (ξ-·₁ ()) 

stuck : Stuck (`zero · `zero)
stuck = ⟨ normalex , notvalueex ⟩  
```
#### Exercise `unstuck` (recommended) ****HOMEWORK****

Provide proofs of the three postulates, `unstuck`, `preserves`, and `wttdgs` above.

```
-- Your code goes here
unstuck m ⟨ n , nv ⟩ with progress m 
... | step  m->n = n m->n
... | done v = nv v

preserves {M} {N} {A} m:a (M ∎) = m:a
preserves {M} {N} {A} m:a (M —→⟨ mm₁ ⟩ m₁n) = preserves (preserve m:a mm₁) m₁n

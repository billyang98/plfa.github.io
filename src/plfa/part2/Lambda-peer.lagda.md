...
```
module plfa.part2.Lambda-peer where
```
...
```
-- Your code goes here
-- Church encoding for multiply : m n f x = m (n f) x
mulᶜ : Term
...
-- Your code goes here
-- Lemma 1
—↠implies—↠′ : ∀ {A B : Term} → (A —↠ B) → (A —↠′ B)
—↠implies—↠′ (M ∎) = refl′ {M}
—↠implies—↠′ (_ —→⟨ L—↠′M ⟩ M—↠′N) = trans′ (step′ L—↠′M) (—↠implies—↠′ M—↠′N)

-- Lemma 2
↠-trans : ∀ {A B C : Term} → (A —↠ B) → (B —↠ C) → (A —↠ C)
↠-trans {A} {B} {C} (M ∎) (B↠C) = (B↠C)
↠-trans {A} {B} {C} ((A) —→⟨ A→X ⟩ (X→B)) (B→C) = ((A) —→⟨ A→X ⟩ (↠-trans X→B B→C))

-- Lemma 3
↠′-implies-↠ : ∀ {A B : Term} → (A —↠′ B) → (A —↠ B)
↠′-implies-↠ (refl′ {M}) = (M ∎)
↠′-implies-↠ (step′ {M} {N} M→N) = ((M) —→⟨ M→N ⟩ (N ∎))
↠′-implies-↠ (trans′ L↠M M↠N) = ↠-trans (↠′-implies-↠ L↠M) (↠′-implies-↠ M↠N)

-- Lemma 4
helper : ∀ {A B C : Term} {x y : B —↠ C} (z : A —→ B) → x ≡ y →(_ —→⟨ z ⟩ x) ≡ (_ —→⟨ z ⟩ y)
helper z refl = refl

-- Lemma 5
↠′-↠-refl : ∀ {A B : Term} → (x : A —↠ B) → (↠′-implies-↠ (—↠implies—↠′ x)) ≡ x
↠′-↠-refl (M ∎) = refl
↠′-↠-refl (_ —→⟨ L→M ⟩ M→N) = (helper L→M (↠′-↠-refl M→N))

-- Main proof of embedding
—↠≲—↠′ : ∀ {A B : Term} → (A —↠ B) ≲ (A —↠′ B)
—↠≲—↠′ {A} {B} =
   record
     { to = —↠implies—↠′
...
one = `suc `zero
-- One plus one
one-plus-one : plus · one · one —↠ `suc `suc `zero
one-plus-one =
  begin
    plus · one · one
 —→⟨ ξ-·₁ (ξ-·₁ β-μ) ⟩
...
-- Your code goes here
-- Lemma 1
Context-implies-list : ∀ (A : Context) → List (Id × Type)
Context-implies-list ∅ = []
Context-implies-list (A , x ⦂ x₁) = ⟨ x , x₁ ⟩ ∷ (Context-implies-list A)

-- Lemma 2
List-implies-context : ∀ (A : List (Id × Type)) → Context
List-implies-context [] = ∅
List-implies-context (x ∷ xs) = ((List-implies-context xs) , (proj₁ x) ⦂ (proj₂ x) )

-- Lemma 3
context-add : ∀ {A B : Context} {x : Id} {y : Type} → A ≡ B → (A , x ⦂ y) ≡ (B , x ⦂ y)
context-add refl = refl

-- Lemma 4
context-refl : ∀ (A : Context) → List-implies-context (Context-implies-list A) ≡ A
context-refl ∅ = refl
context-refl (c , x ⦂ y) = context-add (context-refl c)

-- Lemma 5
list-add : ∀ {A B : List (Id × Type)} {x : Id} {y : Type} → A ≡ B
              → (⟨ x , y ⟩ ∷ A) ≡ (⟨ x , y ⟩ ∷ B)
list-add refl = refl


-- Lemma 6
list-refl : ∀ (A : List (Id × Type)) → Context-implies-list (List-implies-context A) ≡ A
list-refl [] = refl
list-refl (⟨ x , y ⟩ ∷ xs) = list-add (list-refl xs)

-- Main proof
Context-≃ : Context ≃ List (Id × Type)
Context-≃ =
   record
     { to = Context-implies-list
     ; from = List-implies-context
     ; from∘to = λ {c → context-refl c}
     ; to∘from = λ {x → list-refl x}
     }
```
...
-- Main proof
⊢mul : ∀ {Γ} → Γ ⊢ mul ⦂ `ℕ ⇒ `ℕ ⇒ `ℕ
⊢mul = ⊢μ (⊢ƛ (⊢ƛ (⊢case (⊢` ∋m) ⊢zero ((⊢plus · (⊢` ∋n)) · ((⊢` ∋* · ⊢` ∋x) · (⊢` ∋n'))))))
    where
    ∋m = (S (λ()) Z)
    ∋n' = (S (λ ()) Z)
    ∋n = (S (λ ()) Z)

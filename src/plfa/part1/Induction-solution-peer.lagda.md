...
```
module plfa.part1.Induction-solution-peer where
```
...
*-assoc-r (suc m) n p =
  ...
  ≡⟨⟩
    (n + p) + m * (n + p)
  ≡⟨ cong ((n + p) +_) (*-assoc-r m n p) ⟩
    (n + p) + (m * n + m * p)
  ≡⟨ +-rearrange n p (m * n)  (m * p) ⟩
    n + (p + m * n) + m * p
  ≡⟨ cong (_+ m * p) (cong(n +_) (+-comm p (m * n))) ⟩
    n + (m * n + p) + m * p
  ≡⟨ sym(+-rearrange n (m * n) p (m * p)) ⟩
    (n + m * n) + ( p + m * p)
  ≡⟨⟩
    (suc m) * n + (p + m * p)
  ≡⟨⟩
...
*-comm m (suc n) = 
  ...
  ≡⟨⟩
    m * (suc zero + n)
  ≡⟨ *-assoc-r m (suc zero) n ⟩
    m * (suc zero) + m * n
  ≡⟨ cong (_+ (m * n)) (*-identity m) ⟩
    m + m * n
  ≡⟨ cong (m +_) (*-comm m n) ⟩
    m + n * m
  ≡⟨ _*'_ n m ⟩
...
∸-+-assoc zero (suc n) p rewrite zero-minus p = refl
...
^-distribʳ-* m n (suc p) rewrite *-comm (m ^ suc p)  (n ^ suc p) | ^-distribʳ-*  m n p | *-comm (n * (n ^ p)) (m * (m ^ p)) | sym(*-assoc (m * (m ^ p)) n (n ^ p)) | *-comm (m * (m ^ p)) n | *-comm m n | sym(*-assoc (n * m) (m ^ p)  (n ^ p)) | *-assoc n m (m ^ p) = refl


^-*-assoc : ∀ (m n p : ℕ) → (m ^ n) ^ p ≡ m ^ (n * p)
^-*-assoc m n zero rewrite *-zero n = refl
^-*-assoc m n (suc p) rewrite ^-*-assoc m n p | sym(^-distribˡ-+-* m n (n * p)) | *-comm n (suc p) | *-comm p n = refl
```

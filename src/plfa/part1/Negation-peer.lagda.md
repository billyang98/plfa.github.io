...
open import plfa.part1.Connectives using (η-→)
```
...
... | inj₁ (m<n , ¬m≡n , ¬m>n) =
  inj₁ (<-case , ≡-case , >-case)
    where
      <-case = s≤s m<n
      ≡-case = λ { refl → ¬m≡n refl }
      >-case = λ { (s≤s m>n) → ¬m>n m>n }
... | inj₂ (inj₁ (¬m<n , m≡n , ¬m>n)) =
  inj₂ (inj₁ (<-case , ≡-case , >-case))
    where
      <-case = λ { (s≤s m<n) → ¬m<n m<n }
      ≡-case = cong suc m≡n
      >-case = λ { (s≤s m>n) → ¬m>n m>n }
... | inj₂ (inj₂ (¬m<n , ¬m≡n , m>n)) =
  inj₂ (inj₂ ((λ { (s≤s m<n) → ¬m<n m<n }) , (λ { refl → ¬m≡n refl }) , (s≤s m>n) ))
...
⊎-dual-× =
  record
    { to = λ ¬[n+m] → (λ n → ¬[n+m] (inj₁ n)) , (λ m → ¬[n+m] (inj₂ m))
    ; from = λ { (¬n , ¬m) (inj₁ n) → ¬n n ; (¬n , ¬m) (inj₂ m) → ¬m m }
    ; from∘to = λ ¬[n+m] → extensionality λ { (inj₁ n) → refl ; (inj₂ m) → refl }
    ; to∘from = λ { (¬n , ¬m) → help (η-→ ¬n) (η-→ ¬m) }
    }
```
...
Ans: Yes, we do. Proof is given below

```
reverse : {A B : Set} → (¬ A) ⊎ (¬ B) → ¬ (A × B)
reverse (inj₁ ¬n) (n , m) = ¬n n
reverse (inj₂ ¬m) (n , m) = ¬m m

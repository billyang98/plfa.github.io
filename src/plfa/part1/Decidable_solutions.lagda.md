...
```
module plfa.part1.Decidable_solutions where
```
...
#### Exercise `_<?_` (recommended) ****HOMEWORK****
...
#### Exercise `_≡ℕ?_` (practice) ****HOMEWORK****
...
¬s≡s : ∀ { m n : ℕ } → ¬ (m ≡ n) → ¬ (suc m ≡ suc n)
¬s≡s ¬m≡n m≡n = ¬m≡n (suc-injective m≡n)

_≡ℕ?_  : ∀ (m n : ℕ) → Dec (m ≡ n)
_≡ℕ?_ zero zero = yes refl
_≡ℕ?_ (suc m) zero = no λ () 
_≡ℕ?_ zero (suc n) = no λ ()
_≡ℕ?_ (suc m) (suc n) with m ≡ℕ? n
... | yes m≡n = yes (s≡s m≡n)
... | no ¬m≡n = no (¬s≡s ¬m≡n)
```
...
#### Exercise `erasure` (practice) ****HOMEWORK****
...
#### Exercise `iff-erasure` (recommended) ****HOMEWORK****
...
_⇔-dec_ : ∀ {A B : Set} → Dec A → Dec B → Dec (A ⇔ B)
_⇔-dec_ (yes x) (yes y) = yes (record
  {
    to = λ a → y
  ; from = λ b → x
  })
_⇔-dec_ (yes x) (no ¬y) = no (λ xy → ¬y (_⇔_.to xy x))
_⇔-dec_ (no ¬x) (yes y) = no (λ xy → ¬x (_⇔_.from xy y))
_⇔-dec_ (no ¬x) (no ¬y) = yes (record
  { to = λ x → ⊥-elim (¬x x) 
  ; from = λ y → ⊥-elim (¬y y) 
  })
